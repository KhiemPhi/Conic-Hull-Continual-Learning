...
plt.rcParams.update({
    ...
    "figure.dpi": 200, "savefig.bbox": "tight", "lines.linewidth": 1.4,
})
C = {"stale": "#c44e52", "transport": "#4c72b0", "oracle": "#55a868",
     "eps": "#8172b2", "resid": "#937860", "frozen": "#666666"}
...
def fig1():
    ...
    ax[0].set_xlabel("task"); ax[0].set_ylabel("task-0 cohort accuracy (%)")
    ...
    ax[0].annotate(f"|transport $-$ oracle|\n$\\leq$ {gap:.2f} pts at all {len(t)} stages",
                   xy=(0.97, 0.97), xycoords="axes fraction", ha="right", va="top",
                   fontsize=6.6, bbox=dict(fc="white", ec="none", alpha=0.85, pad=1.2))
    ...
    ax[1].set_xlabel("task"); ax[1].set_ylabel("angle (deg)")
    ax[1].set_title("(b) the drift saturates and stays rotational")
    ax[1].legend(loc="upper left", frameon=False)
    rg = rigidity(ep.mean(0), rd.mean(0))
    ax[1].annotate(f"rigidity {np.nanmin(rg):.0f}-{np.nanmax(rg):.0f}% of drift\n"
                   rf"removed by ONE rotation",
                   xy=(0.97, 0.34), xycoords="axes fraction", ha="right", va="top",
                   fontsize=6.6, bbox=dict(fc="white", ec="none", alpha=0.85, pad=1.2))

    for e in ("pdf", "png"):
        fig.savefig(os.path.join(REPO, f"fig1_gauge.{e}"))
    plt.close(fig)
    print(f"fig1_gauge: max |transport-oracle| = {gap:.4f} pts; "
...
def fig2():
    ...
    assert np.shape(probe) == (2,), f"expected (recon_cos, NCM) pairs, got {np.shape(probe)}"
    rc = np.array([[t[f"lam=0|{c}"][0] for c in cls] for t in T])
    x = np.arange(len(cls))
    ax[0].fill_between(x, rc.min(0), rc.max(0), color=C["transport"], alpha=0.16, linewidth=0)
    ax[0].plot(x, rc.mean(0), "o-", ms=3.4, color=C["transport"])
    ax[0].set_xticks(x); ax[0].set_xticklabels(["identity\n(no map)", "orthogonal", "linear",
                                                "MLP"])
    ax[0].set_ylabel(r"reconstruction $\cos$ to $\phi_0$")
    ax[0].set_title("(a) the drift is affine: MLP buys nothing")
    d = rc.mean(0)
    ax[0].annotate(rf"identity $=\cos({np.degrees(np.arccos(d[0])):.0f}\degree)$",
                   xy=(0.03, d[0]), xycoords=("axes fraction", "data"), fontsize=6.6,
                   va="bottom")
    ax[0].annotate(f"MLP - linear = {d[3]-d[2]:+.3f}", xy=(0.97, 0.12),
                   xycoords="axes fraction", ha="right", fontsize=6.6)
    ...
    xs = np.array([0, 1])
    w = 0.3
    ax[1].bar(xs - w / 2, [rig[0].mean(), rig[50].mean()], w, color=C["resid"],
              label="rigidity (%)")
    ax[1].bar(xs + w / 2, [gc[0].mean(), gc[50].mean()], w, color=C["eps"],
    ...
    ax[1].set_xticks(xs); ax[1].set_xticklabels([r"$\lambda=0$", r"$\lambda=50$"])
    ax[1].set_ylabel("rigidity / Gram corr"); ax[1].set_ylim(0, 132)
    ax[1].set_title("(b) the Gram penalty moves what the geometry predicts")

    a2 = ax[1].twinx()
    a2.spines["top"].set_visible(False)
    a2.errorbar(xs, [orc[0].mean(), orc[50].mean()],
                yerr=[[orc[l].mean() - orc[l].min() for l in (0, 50)],
                      [orc[l].max() - orc[l].mean() for l in (0, 50)]],
                fmt="s-", ms=3.4, color=C["oracle"], capsize=2, label="oracle (feature quality)")
    a2.axhline(frozen, ls="--", lw=1.0, color=C["frozen"])
    a2.annotate(f"frozen backbone {frozen:.1f}", xy=(0.02, frozen), xycoords=("axes fraction", "data"),
                fontsize=6.6, va="bottom", color=C["frozen"],
                bbox=dict(fc="white", ec="none", alpha=0.85, pad=1.0))
    a2.set_ylabel("accuracy (%)"); a2.set_ylim(70, 88)
    h1, l1 = ax[1].get_legend_handles_labels()
    h2, l2 = a2.get_legend_handles_labels()
    ax[1].legend(h1 + h2, l1 + l2, loc="upper center", frameon=False, ncol=3,
                 fontsize=6.2, columnspacing=1.0, handlelength=1.4,
                 bbox_to_anchor=(0.5, 1.02))

    for e in ("pdf", "png"):
        fig.savefig(os.path.join(REPO, f"fig2_affine_gram.{e}"))
    plt.close(fig)
    print(f"fig2_affine_gram: recon {dict(zip(cls, np.round(d,4)))}; "
